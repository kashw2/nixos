          ...
          [
            ...
            (pkgs.writeText "hyprland.lua" ''
              hl.on("hyprland.start", function()
                hl.exec_cmd("${lib.getExe' pkgs.systemd "systemctl"} --user is-active --quiet hyprland-session.target || (${lib.getExe' pkgs.systemd "systemctl"} --user import-environment WAYLAND_DISPLAY XDG_CURRENT_DESKTOP HYPRLAND_INSTANCE_SIGNATURE && ${lib.getExe' pkgs.dbus "dbus-update-activation-environment"} --systemd WAYLAND_DISPLAY XDG_CURRENT_DESKTOP HYPRLAND_INSTANCE_SIGNATURE && ${lib.getExe' pkgs.systemd "systemctl"} --user start hyprland-session.target)")
              ...
              hl.curve("spring",         { type = "bezier", points = { {0.34, 1.4},  {0.64, 1} } })
              ...
              hl.animation({ leaf = "windowsIn",     enabled = true, speed = 5.5,  bezier = "spring",       style = "popin 87%" })
              hl.animation({ leaf = "windowsOut",    enabled = true, speed = 6,    bezier = "easeOutQuint", style = "popin 87%" })
