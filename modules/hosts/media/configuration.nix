{ self, inputs, ... }:
{
  flake.nixosModules.media =
    ...
    }:
    {
      ...
      # Service state that would otherwise be wiped by the rollback-root
      # initrd unit. Each entry is guarded on its service's `enable`
      # option so disabling a service automatically drops its persistence.
      # For services that set `DynamicUser = true` in their systemd unit,
      # the real state lives at `/var/lib/private/<name>` — persisting
      # there avoids fighting the symlink systemd creates at `/var/lib/<name>`.
      # Deluge is intentionally omitted: its `dataDir` is set to
      # `/mnt/torrents`, which lives on a separate (non-impermanent) disk.
