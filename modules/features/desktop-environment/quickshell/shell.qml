...
ShellRoot {
    ...
    function setWeatherCity(name) {
        ...
        shell.weatherCustomCity = trimmed;
        weatherCitySave.city = trimmed;
        weatherCitySave.running = true;
        // If the value didn't change, onWeatherCustomCityChanged won't fire —
        // refetch manually so re-applying the same city still retries the fetch.
        if (!changed) refetchWeather();
    }

    function refetchWeather() {
        shell.weatherForecast = [];
        shell.weatherCondition = "";
        shell.weatherTemp = "";
        shell.weatherLocationName = "";
        weatherCheck.running = false;
        weatherForecastCheck.running = false;
        // Defer running=true to the next event loop tick. Setting false→true
        // in the same call stack races: the `command` binding may not have
        // re-evaluated yet, and Process can treat the rapid toggle as a no-op
        // — leaving the fetch using the previous city.
        Qt.callLater(weatherRestart);
    }

    function weatherRestart() {
        weatherCheck.running = true;
        weatherForecastCheck.running = true;
    }

    onWeatherCustomCityChanged: refetchWeather()
    ...
    Process {
        id: weatherCityLoad
        command: ["sh", "-c", "cat \"$HOME/.config/quickshell/weather-city\" 2>/dev/null"]
        running: true
        stdout: SplitParser {
            onRead: data => {
                var s = data.toString().trim();
                if (s !== "") shell.weatherCustomCity = s;
            }
        }
    }
    ...
    Process {
        ...
        command: ["sh", "-c", "mkdir -p \"$HOME/.config/quickshell\" && printf '%s' \"$1\" > \"$HOME/.config/quickshell/weather-city\"", "sh", city]
    }
    ...
    // 7-day forecast via Open-Meteo. If a custom city is set, geocode it via
